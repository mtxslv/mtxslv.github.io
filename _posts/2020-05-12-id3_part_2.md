...
$$ = {(x_j,l_j)} $$
 contains |_D_| examples belonging to _k_ different classes. Let the count for class '_i_' in a set _S_ be given by 
 $$\nu_i := |(x,l) \in S | l = c_i|$$
 
 . The estimated probability-of-occurence for each class is then given by, 
